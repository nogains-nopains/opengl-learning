...
static const GLfloat g_vertex_buffer_data[] = {
    ...
    GLuint elements[] = {
     ...
     // create an element array and bind it.
    GLuint ebo;
    glGenBuffers(1, &ebo);
  ...
  //       GLint posAttrib = 0;
		...
		glVertexAttribPointer(
			...
			2,                  // size of the g_vertex_buffer_data,  the number of values for that input, which is the same as the number of components of the vec. must be 1,	2, 3, or 4. type specifies the data type of each component,
			GL_FLOAT,           // type
			GL_FALSE,           // normalized between -1.0 and 1.0 (or 0.0 and 1.0 depending on the format)  if they aren't floating point numbers.?
			5*sizeof(float),    // stride, because each vertex consists of 5 floating point attribute values. 
        ...
        glEnableVertexAttribArray(colAttrib); // can be called before or after call of glVertexAttribPointer.
        glVertexAttribPointer(colAttrib, 3, GL_FLOAT, GL_FALSE,
                               5*sizeof(float), (void*)(2*sizeof(float)));
        //The offset of 2*sizeof(float) for the color attribute is there 
        //because each vertex starts with 2 floating point values for the
        //position that it has to skip over.
        // It is a pointer to the first component of the first generic vertex 
